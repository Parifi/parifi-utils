...
export const checkisUserActive = async (subgraphEndpoint: string, userAddress: string) => {
  const subgraphResponse: any = await request(subgraphEndpoint, checkActiveUser(userAddress));
  if (!subgraphResponse) throw new Error('Error While Fechting Wallet for Address');
